...
function _pomodoroRedraw(seconds, overlay, text)
	...
	if seconds == 0 or not pomoActive then
		...
		text = hs.styledtext.new(
			time,
			{ font = { name = "Fira Code", size = 150 }, color = text_color, paragraphStyle = { alignment = "center" } }
		)
		...
		hs.timer.doAfter(1, function()
			_pomodoroRedraw(seconds - 1, overlay, textdraw)
		end)
	end
end
...
w = hs.spaces.watcher.new(function(x)
	if pomoActive then
		overlay:hide()
		overlay:show()
	end
